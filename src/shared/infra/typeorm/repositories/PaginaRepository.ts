...
findById(cod_page_uuid: string): Promise<Entities | undefined>;
...
class PaginaRepository implements IRepository {
  ...
  private ormRepository: Repository<Entities>;
    ...
    public async findById(cod_page_uuid: string): Promise<Entities | undefined> {
      this.ormRepository = getRepository(Entities);
      const result = await this.ormRepository.findOne({cod_page_uuid});
      return result;
  }
  public async create({
